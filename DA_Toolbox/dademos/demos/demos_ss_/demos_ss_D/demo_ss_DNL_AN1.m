...
% with additive noise ss_DNL_AN

clc;

disp('This is a demo of how to create and manipulate a nonlinear ');
disp('state space model with additive noise (ss_DNL_AN). It is assumed that ');
disp('the ''demo_ss_DL'' demo has already been studied by the user.');
disp('Note that in its most general form the model equations are:');
disp(' ');
disp(' x(k+1) = f(x(k),u(k),k) + w(k) ');
disp(' y(k)   = h(x(k),u(k),k) + v(k) ');
disp(' ');
...
disp('The function handles for f is @demo_DNL_AN_f and is configured as');
disp(' ');
disp('function x1 = demo_DNL_AN_f(x,t,u,~,Ts)');
disp(' ');
disp('	x1(1,1) = x(1)+2*x(2) + u;');
disp('	x1(2,1) = x(1)+x(2);');
disp(' ');
disp('end');
disp(' ');
disp('The function handle for h is @demo_DNL_AN_h and is configured as');
disp(' ');
disp('function y = demo_DNL_AN_h(x,t,u,~,Ts)');
disp(' ');
disp('	y(1,1) = x(1);');
disp('	y(2,1) = x(2);');
disp(' ');
disp('end');
disp(' ');
disp('Note that the ~ stands for respectively w and v which are not');
disp('used here. They will be used in the ss_DNL model.');
disp(' ');
disp('press <ENTER> key'); pause
...
disp(' ');
disp('>>ssObj=ss_DNL_AN(@demo_DNL_AN_f,@demo_DNL_AN_h,x0,w,v);');
ssObj=ss_DNL_AN(@demo_DNL_AN_f,@demo_DNL_AN_h,x0,w,v);
disp(' ');
disp('The more general way to generate a DNL_AN state space model is ');
disp('briefly explained now to summarize all possible arguments. The most');
disp('general form is:');
disp(' ');
disp(' ''ss_DNL_AN(@demo_DNL_AN_f,@demo_DNL_AN_h,x0,w,v,k0,Ts,TimeUnit,...');
disp('                            @demo_DNL_AN_fJacX,@demo_DNL_AN_hJacX ) ''');
disp(' ');
disp('where k0 is the initial step number, Ts the sample time and TimeUnit');
disp('defines the unit of the time. The last two parameters are function');
disp('handles that return the Jacobians of f with respect to x and of h with');
disp('respect to x. We will come back to this later.');
...
disp(' ');
disp('function JacX = demo_DNL_AN_fJacX(x,k,u,~,Ts)');
disp(' ');
disp('	JacX = [1 2 ; 1 1] ;');
disp(' ');
disp('end');
...
disp(' ');
disp('>>ssObj.fJacX=@demo_DNL_AN_fJacX;');
ssObj.fJacX=@demo_DNL_AN_fJacX;
disp(' ');
