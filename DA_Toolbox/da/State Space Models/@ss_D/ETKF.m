function daModel=ETKF(ssObj,varargin)
%ETKF performs Ensemble Transform Kalman Filter algorithm on a Discrete  
%    State-Space object
%
%  - Input variable(s) -
...
%  Filter has the option NENSEMBLES or INITENSEMBLES. NENSEMBLES indicates
%  the number of ensembles to use. When NENSEMBLES is used, the ensembles 
%  are selected randomly from the X0 noise model of the SSOBJ. 
...
%  
%  - Construction -          
%  DAMODEL=ETKF(SSOBJ,Y,U,OUTCONF,OPTIONSCELL) performs the Ensemble Transform  
%  Kalman Filter algorithm on a Discrete State-Space object.
%  The parameters U, OUTCONF and OPTIONSCELL can be omitted if desired but
%  the order of the parameters must remain.
%  For example: ETKF(SSOBJ,Y,U,OPTIONSCELL) and ETKF(SSOBJ,Y,OUTCONF) are
%  allowed, but ETKF(SSOBJ,OUTCONF,Y) is not allowed
%
%  DAMODEL=ETKF(SSOBJ,SIMMODEL,OUTCONF,OPTIONSCELL) performs the Ensemble   
%  Transform Kalman Filter algorithm on a Discrete State-Space object using  
%  data from the simulation model SIMMODEL.
%  The parameters OUTCONF and OPTIONSCELL can be omitted if desired but
%  the order of the parameters must remain.
%  For example: ETKF(SSOBJ,SIMMODEL,OPTIONSCELL) and ETKF(SSOBJ,Y,OUTCONF) 
%  are allowed, but ETKF(SSOBJ,SIMMODEL,OPTIONSCELL,OUTCONF) is not allowed
%
% References: 
...
% 
	...
	if ni==2 && isa(varargin{1},'cell')
        ...
        if length(size(varargin))> 2 || (size(varargin,1)>1 && size(varargin,2)>1) 
            error('DA:StateSpaceModels:ss_D:ETKF:cellDim','Cell must be vector sized.')
        else
    ...
    % Ensemble Transform Kalman Filter should be applied on additive noise models
    if isa(ssObj,'ss_DNL') || isa(ssObj,'ss_DNL_AMN')
        warning('DA:StateSpaceModels:ss_D:ETKF:ssWarning','Ensemble Transform Kalman Filter should only be applied on additive noise models.')
    end    
    
    % Ensemble Transform Kalman Filter should be applied on zero mean gaussian noise       
    if ~iszeromean(mean(ssObj.w,k0)) || ~iszeromean(mean(ssObj.v,k0)) || ~isa(ssObj.w,'nm_gauss') || ~isa(ssObj.v,'nm_gauss')
        warning('DA:StateSpaceModels:ss_D:ETKF:nonZeroMean','Noise models should be zero mean and Gaussian for ETKF.')
    end
    ...
    if isa(varargin{1},'double')
        ...
        error('DA:StateSpaceModels:ss_D:ETKF:classMismatch','Class mismatch for y: expected sim_D object or double.')
    end
        ...
        if ni>0
            ...
            elseif isa(optionsCell{1},'double')&&length(n)==2&&n(1)>1&&n(2)>1       %matrix 
                ...
                if size(initEnsembles,1)~=ssObj.x0
                    error('DA:StateSpaceModels:ss_D:ETKF:initEnsembles','Number of states of initEnsembles is incorrect.')
                end  
            elseif isempty(optionsCell{1})
                ...
                error('DA:StateSpaceModels:ss_D:ETKF:argMismatch','Optionscell must contain nEnsembles or initEnsembles as first argument.')
            end 
        else
    ...
    if nEnsembles<2
        error('DA:StateSpaceModels:ss_D:ETKF:nEnsembles','Number of ensembles must be minimal equal to two.')
    end    
    ...
    if ~isequal(u,0) && ~isempty(u) && size(u,2)~=size(y,2)
        warning('DA:StateSpaceModels:ss_D:ETKF:uSize','Amount of inputs u is not consistent with amount of measurements y. Smallest amount is used.')
    ...
    %==================================================%
    %    ENSEMBLE TRANSFORM KALMAN FILTER ALGORITHM    %
    %==================================================%
    ...
    for i=2:samples
        ...
        xf=mean(xhat_ens,2);                           	%estimate xf: mean of ensembles
        yf=mean(yf_ens,2);                              %estimate yf: mean of meas. ensembles
        
        Ex=xhat_ens-repmat(xf,1,nEnsembles);           	%ensemble perturbation matrix (nxN)
        Ey=yf_ens-repmat(yf,1,nEnsembles);              %ensemble error matrix of output error (mxN)     
        
        Sx = Ex / sqrt(nEnsembles-1);                   %square root of Pf=Pxx (nxN)
        Sy = Ey / sqrt(nEnsembles-1);                   %square root of Pyy (mxN)
        
        % square root of R  (mxm)        
        Lk = cholcov(ssObj.v,kIndex(i))'; 
        F = Sy' / Lk';                                  % F=(Nxm)
        T = F'*F + eye(nOutputs);                       % T=(mxm)
        ...
        %ANALYSIS STEP
        xhat = xf + K * ( Lk \ (y(:,i) - eval_htot(ssObj,xf,kIndex(i),getU(u,i),muV)));
        [V,D]=eig(F*F');                                        % (NxN)
        vec = diag(D+eye(nEnsembles));
        Sx = (Sx * V / diag(cholSigma(vec,1,2,1))) * V' ;       % (nxN)
        
        xhat_ens = sqrt(nEnsembles - 1) * Sx + repmat(xhat,1,nEnsembles);  %xa_ens
    ...
    % + to change to double
    daModel = dam_D('ETKF',xtrue,xfDa,xaDa,PfDa,PaDa,u ,y ,kIndex,+(covFull),Ts,ssObj.TimeUnit); 
