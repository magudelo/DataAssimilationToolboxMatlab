function daModel=EnSRF(ssObj,varargin)
%EnSRF performs Ensemble Square Root Filter algorithm on a Discrete  
%    State-Space object
%
%  - Input variable(s) -
...
%  Default: 'xa'
%
%  OPTIONSCELL: cell that contains additional options. The Ensemble Square 
%  Root Filter has the option NENSEMBLES or INITENSEMBLES. NENSEMBLES 
...
%  
%  - Construction -          
%  DAMODEL=EnSRF(SSOBJ,Y,U,OUTCONF,OPTIONSCELL) performs the Ensemble  
%  Square Root Filter algorithm on a Discrete State-Space object.
%  The parameters U, OUTCONF and OPTIONSCELL can be omitted if desired but
%  the order of the parameters must remain.
%  For example: EnSRF(SSOBJ,Y,U,OPTIONSCELL) and EnSRF(SSOBJ,Y,OUTCONF) are
%  allowed, but EnSRF(SSOBJ,OUTCONF,Y) is not allowed
%
%  DAMODEL=EnSRF(SSOBJ,SIMMODEL,OUTCONF,OPTIONSCELL) performs the Ensemble   
...
%  the order of the parameters must remain.
%  For example: EnSRF(SSOBJ,SIMMODEL,OPTIONSCELL) and EnSRF(SSOBJ,Y,OUTCONF) 
%  are allowed, but EnSRF(SSOBJ,SIMMODEL,OPTIONSCELL,OUTCONF) is not allowed
%
% References: 
...
% 
	...
	if ni==2 && isa(varargin{1},'cell')
        ...
        if length(size(varargin))> 2 || (size(varargin,1)>1 && size(varargin,2)>1) 
            error('DA:StateSpaceModels:ss_D:EnSRF:cellDim','Cell must be vector sized.')
        else
    ...
    % Ensemble Square Root Filter should be applied on additive noise models
    if isa(ssObj,'ss_DNL') || isa(ssObj,'ss_DNL_AMN')
        warning('DA:StateSpaceModels:ss_D:EnSRF:ssWarning','Ensemble Square Root Filter should only be applied on additive noise models.')
    end    
    
    % Ensemble Square Root Filter should be applied on zero mean gaussian noise       
    if ~iszeromean(mean(ssObj.w,k0)) || ~iszeromean(mean(ssObj.v,k0)) || ~isa(ssObj.w,'nm_gauss') || ~isa(ssObj.v,'nm_gauss')
        warning('DA:StateSpaceModels:ss_D:EnSRF:nonZeroMean','Noise models should be zero mean and Gaussian for EnSRF.')
    end
    ...
    if isa(varargin{1},'double')
        ...
        error('DA:StateSpaceModels:ss_D:EnSRF:classMismatch','Class mismatch for y: expected sim_D object or double.')
    end
        ...
        if ni>0
            ...
            elseif isa(optionsCell{1},'double')&&length(n)==2&&n(1)>1&&n(2)>1       %matrix 
                ...
                if size(initEnsembles,1)~=ssObj.x0
                    error('DA:StateSpaceModels:ss_D:EnSRF:initEnsembles','Number of states of initEnsembles is incorrect.')
                end  
            elseif isempty(optionsCell{1})
                ...
                error('DA:StateSpaceModels:ss_D:EnSRF:argMismatch','Optionscell must contain nEnsembles or initEnsembles as first argument.')
            end 
        else
    ...
    if nEnsembles<2
        error('DA:StateSpaceModels:ss_D:EnSRF:nEnsembles','Number of ensembles must be minimal equal to two.')
    end    
    ...
    if ~isequal(u,0) && ~isempty(u) && size(u,2)~=size(y,2)
        warning('DA:StateSpaceModels:ss_D:EnSRF:uSize','Amount of inputs u is not consistent with amount of measurements y. Smallest amount is used.')
    ...
    %=============================================%
    %    ENSEMBLE SQUARE ROOT FILTER ALGORITHM    %
    %=============================================%
    ...
    for i=2:samples
        ...
        %ANALYSIS STEP        
        K=zeros(nStates,nOutputs);
        KTilde=K;
        R=var(ssObj.v,kIndex(i));
        yest=eval_htot(ssObj,xhat,kIndex(i),getU(u,i),muV);

        for j=1:nOutputs
        
            F = Ey(j,:)' / sqrt(nEnsembles-1);                  %F=(Nx1)
            alpha = 1/(F'*F+Pyy(j,j));                          %alpha=scalar
            K(:,j) = alpha * Sx * F;                            %K(:,j)=(nx1)
            xhat = xhat + K(:,j)*(y(j,i) - yest(j));
            
            beta = 1 / ( 1 + sqrt( alpha*R(j) ) );              %beta=scalar
            KTilde(:,j) =  beta *  K(:,j);                      %KTilde=(nx1)
            Sx = Sx - KTilde(:,j)*F';                           %Sx=(nxN)
        end 

        xhat_ens = Sx / sqrt(nEnsembles-1) + repmat(xhat,1,nEnsembles);  %xa_ens
    ...
    % + to change to double
    daModel = dam_D('EnSRF',xtrue,xfDa,xaDa,PfDa,PaDa,u ,y ,kIndex,+(covFull),Ts,ssObj.TimeUnit);   
