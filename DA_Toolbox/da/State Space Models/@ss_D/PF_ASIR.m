function daModel=PF_ASIR(ssObj,varargin)
%PF_ASIR performs the Auxiliary Particle Filter algorithm on a Discrete 
%     State-Space object
%
%  - Input variable(s) -
...
%  
%  - Construction -          
%  DAMODEL=PF_ASIR(SSOBJ,Y,U,OUTCONF,OPTIONSCELL) performs the Auxiliary 
%  Particle Filter algorithm on a Discrete State-Space object.
%  The parameters U, OUTCONF and OPTIONSCELL can be omitted if desired but
%  the order of the parameters must remain.
%  For example: PF_ASIR(SSOBJ,Y,U,OPTIONSCELL) and PF_ASIR(SSOBJ,Y,OUTCONF) are
%  allowed, but PF_ASIR(SSOBJ,OUTCONF,Y) is not allowed
%
%  DAMODEL=PF_ASIR(SSOBJ,SIMMODEL,OUTCONF,OPTIONSCELL) performs the Auxiliary  
...
%  the order of the parameters must remain.
%  For example: PF_ASIR(SSOBJ,SIMMODEL,OPTIONSCELL) and PF_ASIR(SSOBJ,Y,OUTCONF) 
%  are allowed, but PF_ASIR(SSOBJ,SIMMODEL,OPTIONSCELL,OUTCONF) is not allowed
%
% References: 
...
% University, Sweden - Jeroen D. Hol
	...
	if ni==2 && isa(varargin{1},'cell')
        ...
        if length(size(varargin))> 2 || (size(varargin,1)>1 && size(varargin,2)>1) 
            error('DA:StateSpaceModels:ss_D:PF_ASIR:cellDim','Cell must be vector sized.')
        else
    ...
    % Auxiliary Particle Filter should be applied on additive measurement noise models
    if isa(ssObj,'ss_DNL')
        warning('DA:StateSpaceModels:ss_D:PF_ASIR:ssWarning','Auxiliary Particle Filter should only be applied on additive measurement noise models.')
    end    
    ...
    if isa(varargin{1},'double')
        ...
        error('DA:StateSpaceModels:ss_D:PF_ASIR:classMismatch','Class mismatch for y: expected sim_D object or double.')
    end
    ...
    if ~isempty(optionsCell)
        ...
        if ni>0
            ...
            elseif isa(optionsCell{1},'double')&&length(n)==2&&n(1)>1&&n(2)>1       %matrix 
                ...
                if size(initParticles,1)~=ssObj.x0
                    error('DA:StateSpaceModels:ss_D:PF_ASIR:initParticles','Number of states of initParticles is incorrect.')
                end  
            elseif isempty(optionsCell{1})
                nParticles = 100; 
                initParticles=0;
            else
                error('DA:StateSpaceModels:ss_D:PF_ASIR:argMismatch','Optionscell must contain scalar nParticles or initParticles as first argument.')
            end  
        else
            nParticles = 100;  
            initParticles=0;
        ...
        if ni>1
            ...
            elseif isempty(optionsCell{2})
                ...
                error('DA:StateSpaceModels:ss_D:PF_ASIR:argMismatch','Optionscell must contain Resampler string as second argument.')
            end     
        else
            resampler = 'stratisfied'; 
        end         
        if strcmp(resampler,'residual')&&ni>2
            ...
            elseif isempty(optionsCell{3})
                ...
                error('DA:StateSpaceModels:ss_D:PF_ASIR:argMismatch','Optionscell must contain Resampler string as third argument.')
            end     
        else
            resampler2 = 'stratisfied'; 
        end               
    ...
    if nParticles<2
        error('DA:StateSpaceModels:ss_D:PF_ASIR:nParticles','Number of particles must be minimal equal to two.')
    end
    ...
    if ~isequal(u,0) && ~isempty(u) && size(u,2)~=size(y,2)
        warning('DA:StateSpaceModels:ss_D:PF_ASIR:uSize','Amount of inputs u is not consistent with amount of measurements y. Smallest amount is used.')
	...
	for i=2:samples
        ...
        for j=1:nParticles  %forecast every particle  
            xhat_par(:,j) = eval_ftot(ssObj,xhat_par(:,j),kIndex(i-1),getU(u,i-1),muW);   % (nxN) mu: expected value, no noise 
            yf_par(:,j) = eval_htot(ssObj,xhat_par(:,j),kIndex(i),getU(u,i),muV);       % (mxN)
        end        
        
        %ANALYSIS STEP        
        %Assign the particle the weights w_i
        pzxf = pdf(ssObj.v,repmat(y(:,i),1,nParticles)-yf_par,kIndex(i));              %p(z|xf):needed in second weighting
        wi=pzxf.*wi;	% Calculate weights
        sumwi=sum(wi);        
        if sumwi <= realmin
            error('DA:StateSpaceModels:ss_D:PF_ASIR:sumWi','The sum of weights is zero. Please adjust the parameters or the method.')
        end
        wi=wi/sumwi;                                                                    %Normalize weights
	
        %resample
        resamplerHandle=str2func(strcat('PFRS_',resampler));
        [~,~,indj]=feval(resamplerHandle,[],wi,length(wi),resampler2);                  %Resample: get index
        resampled=0;                                                                    %weights are not initialized      
        
        xhat_par = xhat_par(:,indj) + sample(ssObj.w,nParticles,kIndex(i-1));         	%add noise: as SIR
        ...
        for j=1:nParticles                                                              %measure every particle  
            yf_par(:,j) = eval_htot(ssObj,xhat_par(:,j),kIndex(i),getU(u,i),muV);         % (mxN)
        end          
        wi=pdf(ssObj.v,repmat(y(:,i),1,nParticles)-yf_par,kIndex(i)) ./ pzxf(indj);
        sumwi=sum(wi);        
        if sumwi <= realmin
            error('DA:StateSpaceModels:ss_D:PF_ASIR:sumWi','The sum of weights is zero. Please adjust the parameters or the method.')
        end
    ...
    % + to change to double
    daModel = dam_D('PF_ASIR',xtrue,xfDa,xaDa,PfDa,PaDa,u ,y ,kIndex,+(covFull),Ts,ssObj.TimeUnit);  
