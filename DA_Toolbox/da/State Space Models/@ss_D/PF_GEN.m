function daModel=PF_GEN(ssObj,varargin)
%PF_GEN performs Generic Particle Filter algorithm on a Discrete State-Space  
%    object
%
%  - Input variable(s) -
...
%  equal to 100.
%  The second variable is Nt, the effective sample size treshold for 
%  resampling expressed in a scale from 0 to 1 x NPARTICLES. Resampling 
%  occurs when Neff < (Nt x NPARTICLES). Default = 1.
%  The third variable is the string RESAMPLER which indicates the resample
...
%  When the third variable is selected as 'residual' a fourth variable can
...
%  
%  - Construction -          
%  DAMODEL=PF_GEN(SSOBJ,Y,U,OUTCONF,OPTIONSCELL) performs the Generic Particle 
...
%  For example: PF_GEN(SSOBJ,Y,U,OPTIONSCELL) and PF_GEN(SSOBJ,Y,OUTCONF) are
%  allowed, but PF_GEN(SSOBJ,OUTCONF,Y) is not allowed
%
%  DAMODEL=PF_GEN(SSOBJ,SIMMODEL,OUTCONF,OPTIONSCELL) performs the Generic  
...
%  the order of the parameters must remain.
%  For example: PF_GEN(SSOBJ,SIMMODEL,OPTIONSCELL) and PF_GEN(SSOBJ,Y,OUTCONF) 
%  are allowed, but PF_GEN(SSOBJ,SIMMODEL,OPTIONSCELL,OUTCONF) is not allowed
%
% References: 
...
% University, Sweden - Jeroen D. Hol
	...
	if ni==2 && isa(varargin{1},'cell')
        ...
        if length(size(varargin))> 2 || (size(varargin,1)>1 && size(varargin,2)>1) 
            error('DA:StateSpaceModels:ss_D:PF_GEN:cellDim','Cell must be vector sized.')
        else
    ...
    % Generic Particle Filter should be applied on additive measurement noise models
    if isa(ssObj,'ss_DNL')
        warning('DA:StateSpaceModels:ss_D:PF_GEN:ssWarning','Generic Particle Filter should only be applied on additive measurement noise models.')
    end    
    ...
    if isa(varargin{1},'double')
        ...
        error('DA:StateSpaceModels:ss_D:PF_GEN:classMismatch','Class mismatch for y: expected sim_D object or double.')
    end
    ...
    if ~isempty(optionsCell)
        ...
        if ni>0
            ...
            elseif isa(optionsCell{1},'double')&&length(n)==2&&n(1)>1&&n(2)>1       %matrix 
                ...
                if size(initParticles,1)~=ssObj.x0
                    error('DA:StateSpaceModels:ss_D:PF_GEN:initParticles','Number of states of initParticles is incorrect.')
                end  
            elseif isempty(optionsCell{1})
                nParticles = 100; 
                initParticles=0;
            else
                error('DA:StateSpaceModels:ss_D:PF_GEN:argMismatch','Optionscell must contain scalar nParticles or initParticles as first argument.')
            end  
        else
            nParticles = 100;  
            initParticles=0;
        ...
        if ni>1
            ...
            elseif isempty(optionsCell{2})
                ...
                error('DA:StateSpaceModels:ss_D:PF_GEN:argMismatch','Optionscell must contain scalar Nt as second argument.')
            end        
        else
            Nt = 1;  
        end       
        if ni>2
            if isa(optionsCell{3},'char')                                           %string
                resampler=optionsCell{3};
            elseif isempty(optionsCell{3})
                resampler = 'stratisfied';                
            else
                error('DA:StateSpaceModels:ss_D:PF_GEN:argMismatch','Optionscell must contain Resampler string as third argument.')
            end     
        else
            resampler = 'stratisfied'; 
        end         
        if strcmp(resampler,'residual')&&ni>3
            if isa(optionsCell{4},'char')                                           %string
                resampler2=optionsCell{4};
            elseif isempty(optionsCell{4})
                resampler2 = 'stratisfied';                
            else
                error('DA:StateSpaceModels:ss_D:PF_SIR:argMismatch','Optionscell must contain Resampler string as fourth argument.')
            end     
        else
            resampler2 = 'stratisfied'; 
        end                 
    ...
    if nParticles<2
        error('DA:StateSpaceModels:ss_D:PF_GEN:nParticles','Number of particles must be minimal equal to two.')
    end
    ...
    if ~isequal(u,0) && ~isempty(u) && size(u,2)~=size(y,2)
        warning('DA:StateSpaceModels:ss_D:PF_GEN:uSize','Amount of inputs u is not consistent with amount of measurements y. Smallest amount is used.')
    ...
    for i=2:samples
        ...
        wi=pdf(ssObj.v,repmat(y(:,i),1,nParticles)-yf_par,kIndex(i)).*wi;	% Calculate weights
        sumwi=sum(wi);     
        if sumwi <= realmin
            error('DA:StateSpaceModels:ss_D:PF_GEN:sumWi','The sum of weights is zero. Please adjust the parameters or the method.')
        end
        wi=wi/sumwi;                                                      % Normalize weights
	
        %resample if Neff < Nt * nParticles
        if 1/sum(wi.^2) < Nt * nParticles 
    ...
    % + to change to double
    daModel = dam_D('PF_GEN',xtrue,xfDa,xaDa,PfDa,PaDa,u ,y ,kIndex,+(covFull),Ts,ssObj.TimeUnit);  
